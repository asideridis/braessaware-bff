...
var settings = DownstreamSettings.FromConfiguration(builder.Configuration, "user");
...
app.MapGet("/user", async (HttpContext context, CancellationToken cancellationToken) =>
{
    ...
    double BaseErrorRate)
{
    public static DownstreamSettings FromConfiguration(IConfiguration configuration, string serviceName)
    {
        ...
        var primaryDelay = configuration.GetValue("PRIMARY_DELAY_MS", 50);
        var alternateDelay = configuration.GetValue("ALTERNATE_DELAY_MS", 30);
        var degradedPenalty = configuration.GetValue("DEGRADED_PENALTY_MS", 400);
        var jitter = configuration.GetValue("JITTER_MS", 20);
