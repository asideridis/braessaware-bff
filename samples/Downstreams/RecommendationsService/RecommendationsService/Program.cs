...
var settings = DownstreamSettings.FromConfiguration(builder.Configuration, "recommendations");
...
app.MapGet("/recommendations", async (CancellationToken cancellationToken) =>
{
    ...
    double BaseErrorRate)
{
    public static DownstreamSettings FromConfiguration(IConfiguration configuration, string serviceName)
    {
        ...
        var primaryDelay = configuration.GetValue("PRIMARY_DELAY_MS", 120);
        var alternateDelay = configuration.GetValue("ALTERNATE_DELAY_MS", 60);
        var degradedPenalty = configuration.GetValue("DEGRADED_PENALTY_MS", 500);
        var jitter = configuration.GetValue("JITTER_MS", 50);
